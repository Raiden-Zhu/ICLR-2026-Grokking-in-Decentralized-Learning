...
def load_pickle_splited_data(loader_path):
    import pickle

    # Load
    with open(os.path.join(loader_path), "rb") as f:
        saved_data = pickle.load(f)

    # Reconstruct
    from torch.utils.data import DataLoader

    reconstructed = [DataLoader(**data) for data in saved_data]
    split_point = len(reconstructed) // 2
    train_dataloaders = reconstructed[:split_point]
    valid_dataloaders = reconstructed[split_point:]
    return train_dataloaders, valid_dataloaders


def get_gradient_vector(network):
    """
    Calculate the gradient vector for a batch without updating the model.
    Parameters:
    ...
    return torch.cat(grad_vector).to(torch.float16)


def get_mean_gradient_from_dataloader(network, dataloader, restore_grad=True):
    """
    Calculate the mean gradient vector for the first batch from a dataloader.
    Parameters:
    network: torch.nn.Module
        The neural network model
    dataloader: torch.utils.data.DataLoader
        DataLoader containing the dataset
        The dataloader should have shuffle=True to ensure random sampling
    restore_grad: bool
        Whether to restore original gradients after computation
    Returns:
    torch.Tensor
        Flattened mean gradient vector
    """
    if restore_grad:
    ...
    mean_grad_vector = grad_vector / len(inputs)
    if restore_grad:
        # Restore original gradients
        for param, orig_grad in zip(network.parameters(), original_grads):
            if orig_grad is not None:
                param.grad = orig_grad
            else:
                param.grad = None
    return mean_grad_vector
